 ...
 * to the input value `x`. If `x` is 0, the function returns 1.
 *
 ...
 *
 * @note This function uses bitwise operations to efficiently compute the result.
 *       It assumes that the input is within the range of a 32-bit unsigned integer.
 */
...
/**
 * @brief Constrains an index to fit within the bounds of a given size by wrapping it around.
 *
 * This function ensures that the provided index remains within the range [0, size - 1]
 * by using the modulo operation. It is useful for cyclic or circular indexing.
 *
 * @param idx The index to be constrained.
 * @param size The size of the range within which the index should be constrained.
 ...
 * @return Eigen::ArrayXd A 1D array containing linearly spaced values from 0 to num-1.
 */
 ...
 * in the array is determined by the difference between `high` and `low` plus one.
 * 
 * @param low The starting value of the range (inclusive).
 * @param high The ending value of the range (inclusive).
 * @return Eigen::ArrayXd A 1D array of linearly spaced values from `low` to `high`.
 */
