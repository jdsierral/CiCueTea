...
NsgfCqtCommon::NsgfCqtCommon(double sampleRate, Eigen::Index blockSize,
                             double fraction, double minFrequency,
                             double maxFrequency, double refFrequency)
    : fs(sampleRate), nSamps(blockSize), frac(fraction), fMin(minFrequency),
  ...
  bax =
      fRef *
      (frac * log(2) * regspace(-bandInfo.nBandsDown, bandInfo.nBandsUp)).exp();
...
NsgfCqtFull::NsgfCqtFull(double sampleRate, Index numSamples, double fraction,
                         double minFrequency, double maxFrequency,
                         double refFrequency)
    : NsgfCqtCommon(sampleRate, numSamples, fraction, minFrequency,
                    maxFrequency, refFrequency),
      Xmat(nSamps, nBands) {
